<template>
	<view class="smallBg">
		...
		<view style="display: flex;flex-direction: column;align-items: center;justify-content: space-between;">
			<block v-if="type != 'home'"><!-- <view class="spacing">{{item.shoseName}}</view> --></block>
			<view class="spacing">
				<text>
			...
			<block v-if="type != 'home'">
				...
				<view v-if="DownPrice" class="spacing" :style="{ color: item.shoseChange > 0 ? 'green' : 'red' }">
					比前一天{{ item.shoseChange > 0 ? '增' : '降' }}{{ item.shoseChange | MathAbs }}
				</view>
				<view class="spacing spacingBetween">
